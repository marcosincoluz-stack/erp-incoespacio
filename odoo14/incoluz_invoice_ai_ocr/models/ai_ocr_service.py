...
class AiOcrService:
    ...
    def analyze_invoice_document(cls, env, file_bytes, filename=""):
        ...
        api_key = icp.get_param('incoluz_invoice_ocr.gemini_api_key', default='').strip()
        if not api_key:
            raise UserError(
        ...
        model = icp.get_param('incoluz_invoice_ocr.gemini_model', default='gemini-2.5-flash').strip() or 'gemini-2.5-flash'
        ...
        payload = {
            "systemInstruction": {"parts": [{"text": INVOICE_PROMPT_SYSTEM}]},
            "contents": [{
                "parts": [
        ...
        try:
            ...
            raise UserError("Error de conexión con la IA de Google tras reintentos: %s" % str(e))
        ...
        if response is None or response.status_code != 200:
            ...
            err_msg = "Error al procesar la factura con Gemini (Código %s)" % status
            try:
                err_json = response.json()
                if "error" in err_json and "message" in err_json["error"]:
                    err_msg += ": %s" % err_json['error']['message']
            except Exception:
                err_msg += ": %s" % body_text[:200]
        ...
        try:
            ...
            # Limpiar posibles bloques de markdown (py3.7: sin removeprefix)
            ...
            raise UserError("No se pudo interpretar el resultado de la IA: %s" % str(e))
